...
let builtInDisabled=toscalar('${exclude}${built_in_name}' in (DisabledParsers) or 'Any' in (DisabledParsers)); 
union isfuzzy=true
${related_parsers}
'''
...
class Parser:
    ...
    def create_query_file(self, name, query):
        for parserName in PARSERS_NAMES:
            query = query.replace(parserName, "_" + parserName)
...
class Main_Parser(Parser):
    ...
    def init_parser(self):
        ...
        if(PARSER_PARAMS in parsed_yaml):
            self.parameters = parsed_yaml[PARSER_PARAMS]
...
class Schema:
    ...
    def get_all_schema_parsers(self):
        ...
        func = lambda parserfile: parserfile.replace(".yaml", "")
        return map(func, all_parsers_files)
...
def main():
    ...
    PARSERS_NAMES = []
    schemas = [Schema("Dns", "ASimDns"), Schema("NetworkSession", "ASimNetworkSession"), Schema("WebSession", "ASimWebSession"), Schema("ASim", "../ASIM", "Library/Functions", "Function")]
    for schema in schemas:
        for f in schema.get_all_schema_parsers():
            print(f)
            PARSERS_NAMES.append(f)
